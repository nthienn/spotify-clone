...
const { isPlaying, currentTrack, currentAlbum } = storeToRefs(useSong);

const playFunc = () => {
  if (currentTrack.value) {
    useSong.playOrPauseThisSong(currentAlbum.value, currentTrack.value);
    return;
  }
...
<template>
  <div class="p-8 overflow-x-hidden">
    ...
    <div class="flex items-center relative w-full h-full">
      <img width="140" :src="album.albumCover" />

      <div class="w-full ml-5">
        <div
          class="text-white absolute text-[33px] w-full hover:underline cursor-pointer top-0 font-semibold"
        >
          {{ album.name }}
        </div>
        ...
        <div class="text-gray-300 text-[13px] flex">
          ...
          <div class="ml-2 flex">
            ...
            <span class="-ml-0.5">{{ album.releaseYear }}</span>
          </div>
          <div class="ml-2 flex">
            <div class="circle mt-2 mr-2"></div>
            <span class="-ml-0.5">{{ album.tracks.length }} songs</span>
          </div>
        </div>
    ...
    <ul class="w-full" v-for="(track, index) in album.tracks" :key="index">
